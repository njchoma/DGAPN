...
class sslData(Dataset):
    def __init__(self, smiles, keep_frac):
    ...
    def __getitem__(self, index):
        ...
        # Obscuring bonds happens here:
        num_bonds = len(mol.GetBonds())
        keep_idx = int(self.keep_frac * num_bonds)
        bonds_idx_shuffled = np.random.permutation(num_bonds)
        g['edge_attr'] = g['edge_attr'][bonds_idx_shuffled][:keep_idx]
        g['edge_index'] = g['edge_index'][bonds_idx_shuffled][:keep_idx]

        # Target should be a [3, X] matrix denoting the X links we need to predict. Going down a column: source, end, edge attribute.

        target = torch.cat((g['edge_index'][:,bonds_idx_shuffled][:,keep_idx:].float(),\
                            g['edge_attr'][bonds_idx_shuffled].reshape([1,-1])[keep_idx:]),0)
        # print(target.shape)
...
def create_ssl_datasets(smiles, keep_frac, np_seed=0):
    nb_samples = len(smiles)
    ...
    train_data = sslData(smiles[:nb_train], keep_frac)
    valid_data = sslData(smiles[nb_train:nb_train + nb_valid], keep_frac)
    test_data = sslData(smiles[nb_train + nb_valid:], keep_frac)
...
def main(artifact_path,
    ...
    if ssl:
        train_data, valid_data, test_data = create_ssl_datasets(smiles)
    else:
